...
## 标题规范（重要）
- `title` 与页面 H1 必须包含本次运行日期 `{{DAILY_DATE}}`，避免站内列表、浏览器标签页或分享卡片里只看到内容标题而缺少对应日期。
- 推荐格式：`{{DAILY_DATE}}｜<内容导向的精简标题>`；日期之后的内容标题用 15~35 字概括当天最重要 1~2 条主线。
- **禁止**只使用固定栏目名作为标题：`每日科技动态`、`今日科技资讯` 等。
- **禁止**使用只有栏目名 + 日期、但没有内容主线的标题，例如 `每日科技动态 - {{DAILY_DATE}}` 或 `今日科技资讯 - {{DAILY_DATE}}`。
- frontmatter 的 `date` 字段仍必须写 `{{DAILY_DATE}}`；页面 H1 与 `title` 保持一致。

好标题示例：
- `{{DAILY_DATE}}｜Firefox 154 原生化布局，CodeQL 覆盖 Vue`
- `{{DAILY_DATE}}｜OpenAI 零数据保留架构，GitHub 质量趋势看板上线`

差标题示例：
- `每日科技动态 - {{DAILY_DATE}}`（只有栏目名和日期，没有内容主线）
- `每日科技动态`（单独使用固定栏目名）
- `今日科技资讯`
