import { defineConfig, devices } from '@playwright/test';

const MOCK_SUPABASE_URL = 'http://mock-supabase.test';
const MOCK_ANON_KEY = 'mock-anon-key';

export default defineConfig({
  testDir: './e2e',
  fullyParallel: true,
  forbidOnly: Boolean(process.env.CI),
  retries: process.env.CI ? 1 : 0,
  workers: process.env.CI ? 1 : undefined,
  reporter: process.env.CI ? [['github'], ['html', { open: 'never' }]] : 'list',
  use: {
    baseURL: 'http://127.0.0.1:3310',
    trace: 'on-first-retry',
  },
  projects: [
    {
      name: 'chromium',
      use: { ...devices['Desktop Chrome'] },
    },
  ],
  webServer: {
    command: 'pnpm --filter web dev -- --port 3310',
    url: 'http://127.0.0.1:3310',
    reuseExistingServer: false,
    timeout: 180_000,
    env: {
      PORT: '3310',
      NEXT_PUBLIC_SITE_URL: 'http://127.0.0.1:3310',
      NEXT_PUBLIC_CHAT_URL: 'https://chat.acongm.com',
      NEXT_PUBLIC_SUPABASE_URL: MOCK_SUPABASE_URL,
      NEXT_PUBLIC_SUPABASE_ANON_KEY: MOCK_ANON_KEY,
      NEXT_PUBLIC_AUTH_URL: 'https://auth.acongm.com',
      AI_CHATS_UPSTREAM_URL: 'http://127.0.0.1:3310/api/chats',
      USER_API_UPSTREAM_URL: 'http://127.0.0.1:3310/api/user',
      AUTH_SESSION_URL: 'http://127.0.0.1:3310/api/auth/session',
    },
  },
});
