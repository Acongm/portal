...
数据流转:
搭建态：用户操作 -> 编辑器 -> 更新 Schema -> 实时预览（运行态渲染）
运行态：Schema -> 引擎解析 -> 渲染器 -> 实际页面
发布流程：搭建态 Schema -> 校验 -> 持久化存储 -> 运行态加载 -> 渲染
...
3. iframe 隔离：在 iframe 中执行，通过 postMessage 通信
...
4. Worker 隔离：在 Web Worker 中执行，完全隔离 DOM
...
- 吸附算法：
  ...
  return \{
  x: Math.round(x / gridSize) _ gridSize,
  y: Math.round(y / gridSize) _ gridSize
  };
  }
- 智能对齐：
  - 水平对齐：检测垂直方向的边缘对齐
