...
function ChatDrawerPanel({ context, seedMessages, open, onClose }: {
  context: DocChatContext;
  seedMessages?: ChatUiMessage[] | null;
  open: boolean;
  onClose: () => void;
    ...
    <div className="acongm-chat-shell">
      <div className="acongm-chat-shell__header">
        <div className="acongm-chat-shell__titles">
          <h3>AI 阅读助手</h3>
          <p title={context.title || '当前文档'}>{context.title || '当前文档'}</p>
        </div>
        <button type="button" className="acongm-chat-shell__close" onClick={onClose} title="关闭" aria-label="关闭助手">
          <X size={16} strokeWidth={2} aria-hidden />
          <span className="acongm-chat-shell__close-label">关闭</span>
        </button>
      </div>
      <div className="acongm-chat-shell__body">
        <DocChatRuntimeProvider context={context} active={open} seedMessages={seedMessages}>
          <AssistantThread />
...
export function ChatDrawer({ context, seedMessages = null }: ChatDrawerProps) {
  ...
  return (
    <Drawer
      ...
      width={isMobile ? '100%' : isDesktop ? 'var(--acongm-chat-width)' : 'min(520px, 88vw)'}
      ...
      styles={{ mask: { background: 'rgba(20, 22, 31, 0.36)' }, wrapper: isDesktop ? { boxShadow: 'var(--acongm-shadow)' } : undefined }}
    >
      <ChatDrawerPanel context={context} seedMessages={seedMessages} open={open} onClose={closePanel} />
    </Drawer>
  );
}
